...
final class SystemStorageLibHolder {
  private static volatile SystemStorageLib instance;

  static SystemStorageLib getInstance() {
    if (instance == null) {
      synchronized (SystemStorageLibHolder.class) {
        if (instance == null) {
  ...
  private static SystemStorageLib createDefault() {
    try {
      BaseDirectories baseDirs = BaseDirectories.get();
      return SystemStorageLib.create(
